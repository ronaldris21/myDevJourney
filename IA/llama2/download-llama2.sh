...
do
    ...
    if [ "$CPU_ARCH" = "arm64" ]; then
      ...
      (cd ${TARGET_FOLDER}"/${MODEL_PATH}" && md5sum -c checklist.chk)
        ...
        # Solicitar que el usuario ingrese "OK" para terminar el script
    while true; do
        read -p "Ingrese 'OK' para finalizar el script: " userInput
        if [ "$userInput" = "OK" ]; then
            break
        else
            echo "Entrada incorrecta. Por favor, ingrese 'OK' para finalizar el script."
        fi
    done
done
